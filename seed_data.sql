...
-- Policy Governance and Implementation Examples
INSERT INTO knowledge_base (category, subcategory, question, answer, tags) VALUES
('Policy Governance', 'Implementation', 'How do I implement a new data governance policy?', 'To implement a new data governance policy in the platform: (1) Define the policy scope and objectives; (2) Identify affected data elements and sensitivity levels; (3) Specify the purposes for which the data can be used; (4) Configure the appropriate controls and restrictions; (5) Use the DDL Generator to create implementation scripts for Snowflake; (6) Deploy the scripts to your Snowflake environment; (7) Validate the implementation with test cases.', 'implementation,policy,governance,how-to'),
('Policy Governance', 'Best Practices', 'What are best practices for policy governance?', 'Best practices for policy governance in the platform include: (1) Align policies with business purposes; (2) Use purpose-based access control consistently; (3) Implement least privilege principles; (4) Regularly review and update policies; (5) Document policy decisions and rationales; (6) Ensure policies are technically enforceable; (7) Maintain an audit trail of policy changes; (8) Validate policy effectiveness through testing.', 'best practices,governance,policies');

-- Application Usage
INSERT INTO knowledge_base (category, subcategory, question, answer, tags) VALUES
('Application Usage', 'Navigation', 'How do I navigate the application?', 'The application has a main navigation menu on the left side that provides access to all major sections: Core Constructs, Regulatory Metadata, Decision Trees, Inference APIs, and Data Use Governance. Each section has its own sub-navigation. The top bar provides access to user settings, notifications, and help resources. You can use the search function at the top of each page to quickly find specific items.', 'navigation,UI,usage'),
('Application Usage', 'DDL Generation', 'How do I generate DDL scripts?', 'To generate DDL scripts in the application: (1) Navigate to the DDL Generator section; (2) Select the target data assets; (3) Configure the policy parameters including purposes, roles, and masking requirements; (4) Click "Generate DDL"; (5) Review the generated scripts; (6) Download or copy the scripts for implementation in your Snowflake environment. The application will automatically create all necessary roles, masking policies, and access controls.', 'DDL,generation,Snowflake,how-to'),
('Application Usage', 'Knowledge Base', 'How do I use the Knowledge Base feature?', 'The Knowledge Base in this application has two main interfaces: (1) AI Chatbot - Ask natural language questions about data governance concepts and get AI-generated answers based on the knowledge base; (2) Browse Knowledge Base - Select categories and specific questions from dropdown menus to view pre-defined answers. The Knowledge Base covers topics like Core Constructs, Regulatory Metadata, Decision Trees, and Data Use Governance implementation details.', 'knowledge base,help,documentation,chatbot'),
('Application Usage', 'Data Access Requests', 'How do I submit a data access request?', 'To submit a data access request in this application: (1) Navigate to the Data Access section; (2) Click "New Request"; (3) Select the data asset you need access to; (4) Choose the business purpose for your request; (5) Specify the duration needed; (6) Provide justification; (7) Submit the request. The system will automatically route your request to the appropriate approvers based on the data sensitivity and purpose.', 'data access,requests,workflow');

-- App-Specific Implementation
INSERT INTO knowledge_base (category, subcategory, question, answer, tags) VALUES
('App Implementation', 'Architecture', 'How is this application implemented?', 'This application is built using a modern web architecture with: (1) Frontend - Streamlit for the user interface with interactive components; (2) Backend - Python services handling business logic and API integration; (3) Database - MySQL for storing metadata, knowledge base, and configuration; (4) Integration - APIs for connecting with Vertex AI for generative AI features and Snowflake for DDL execution; (5) Authentication - Integration with enterprise identity providers.', 'architecture,implementation,technology stack'),
('App Implementation', 'Vertex AI Integration', 'How does this app use Vertex AI?', 'This application integrates with Google Vertex AI to provide intelligent features: (1) Knowledge Base Chatbot - Uses Vertex AI to generate contextual answers based on the knowledge repository; (2) DDL Generation - Leverages Vertex AI to create optimized Snowflake DDL based on policy specifications; (3) Policy Inference - Uses AI to recommend appropriate policies based on data characteristics; (4) Sensitivity Classification - Automatically classifies data elements based on their content and context.', 'Vertex AI,integration,generative AI,implementation'),
('App Implementation', 'Snowflake Integration', 'How does this app integrate with Snowflake?', 'This application integrates with Snowflake through: (1) DDL Generation - Creates optimized Snowflake DDL scripts for implementing purpose-based access control; (2) Policy Deployment - Can directly deploy generated policies to Snowflake environments; (3) Metadata Synchronization - Imports Snowflake schema metadata to maintain an up-to-date inventory; (4) Access Validation - Verifies that implemented policies in Snowflake match the intended policy specifications.', 'Snowflake,integration,DDL,implementation');
...
-- Implementation Details - Core Constructs
INSERT INTO knowledge_base (category, subcategory, question, answer, tags) VALUES
('Implementation Details', 'Core Constructs', 'How are the core data governance constructs implemented in the database?', 'The application implements core data governance constructs through a relational database schema with these key tables: (1) data_element - fundamental data attributes like names, emails, etc.; (2) data_category - groupings of related data elements; (3) sensitivity - classification levels for data; (4) purpose - business reasons for data processing; (5) policy - rules governing data handling; (6) external_roles - roles from integrated systems like Snowflake; (7) purpose_role - mapping between purposes and roles. These tables are interconnected through mapping tables like policy_purpose_data_element that establish relationships between policies, purposes, and data elements. This structure enables the application to determine which policies apply to specific data elements when used for particular purposes by specific roles.', 'database schema,implementation,core constructs'),
('Implementation Details', 'Relationship Model', 'How do the core constructs relate to each other in the implementation?', 'The core constructs are connected through a relationship model: (1) Purposes are linked to Roles through the purpose_role table, establishing which roles can access data for specific purposes; (2) Policies are connected to Purposes through policy_purpose, defining which policies apply to each purpose; (3) Data Elements are linked to Policies and Purposes through policy_purpose_data_element, specifying how each data element should be handled for each purpose; (4) Data Categories contain multiple Data Elements through data_category_data_element; (5) Sensitivity levels are mapped to policies through sensitivity_policy_mapping, enabling automatic policy derivation based on data sensitivity. This relationship model allows the application to determine the appropriate controls for data elements when accessed for specific purposes by specific roles.', 'relationships,data model,implementation'),
('Implementation Details', 'Policy Types', 'What types of policies are implemented in the system and how are they structured?', 'The system implements three primary policy types, each with dedicated tables: (1) Security Policies (policy_purpose_data_element) - control encryption, masking, and access restrictions with attributes like encryption_required, encryption_algorithm, masking_required, and masking_format; (2) Usage Policies (policy_purpose_data_usage) - govern how data can be used with attributes like operation (read/write/share/delete), allowed (boolean), and restrictions; (3) Retention Policies (policy_purpose_data_retention) - manage data lifecycle with attributes like retention_period and retention_basis. Each policy type is implemented with purpose and data element dimensions, allowing fine-grained control. The system also supports role-specific overrides through tables like policy_override_role_purpose_data_security.', 'policy types,security,usage,retention'),
('Implementation Details', 'Purpose-Role Mapping', 'How does the system implement purpose-based access control with roles?', 'The system implements purpose-based access control through the purpose_role mapping table, which connects legitimate business purposes with external system roles. This table contains: (1) purpose_id - references the business purpose; (2) external_role_id - references roles in systems like Snowflake or Databricks; (3) timestamps for auditing. When a policy is evaluated, the system checks if the requesting role has a legitimate purpose mapping. For example, a Marketing Analyst role (from Snowflake) might be mapped to the "Marketing Campaigns" purpose, allowing access to customer data for that specific purpose. The external_roles table maintains information about roles from integrated systems, including source_system and source_role_name fields.', 'purpose-based access,roles,implementation'),
('Implementation Details', 'Policy Overrides', 'How does the system handle exceptions and overrides to policies?', 'The system implements a policy override mechanism through dedicated tables: (1) policy_override_role_purpose_data_security - for security policy exceptions; (2) policy_override_role_purpose_data_usage - for usage policy exceptions; (3) policy_override_role_purpose_data_retention - for retention policy exceptions. Each override table references policy_purpose_data_element_id and external_role_id, allowing exceptions for specific role-purpose-data combinations. For example, while a general policy might require masking all customer phone numbers, an override might allow Customer Service Representatives to see unmasked phone numbers when handling support cases. These overrides take precedence during policy evaluation, enabling legitimate business exceptions while maintaining audit trails.', 'policy exceptions,overrides,implementation');

-- Policy Inference Engines
INSERT INTO knowledge_base (category, subcategory, question, answer, tags) VALUES
('Policy Inference Engines', 'Sensitivity-Based Inference', 'How does the sensitivity-based policy inference engine work?', 'The sensitivity-based policy inference engine in the Assets page works through these steps: (1) Data Element Sensitivity Determination - For each data element in an asset, the engine determines its sensitivity level using the sensitivity_inference component; (2) Obligation Identification - Based on sensitivity levels, the engine identifies security and privacy obligations that apply to each data element using the sensitivity_obligation table; (3) Policy Derivation - The engine maps these obligations to concrete policies using sensitivity_policy_mapping; (4) Control Generation - Finally, it generates specific security controls like encryption requirements, masking formats, and access restrictions. This approach is implemented in the get_sensitivity_based_policies_for_asset method of AssetPolicyInference class, ensuring appropriate protections are applied based on data sensitivity.', 'sensitivity inference,policy engine,implementation'),
('Policy Inference Engines', 'Purpose-Based Inference', 'How does the purpose-based policy inference engine work?', 'The purpose-based policy inference engine in the Assets page operates through these steps: (1) Context Collection - The engine takes asset_id, purpose_id, policy_type, and role_id as inputs; (2) Catalog Entry Retrieval - It retrieves all data elements (catalog entries) for the specified asset; (3) Purpose-Policy Mapping - For each data element and purpose combination, it queries the policy_purpose_data_element table to find applicable policies; (4) Role Filtering - If a specific role is provided, it filters policies based on purpose-role mappings; (5) Policy Type Processing - It processes each policy type (security, usage, retention) separately, retrieving specific policy details from respective tables; (6) Override Application - It checks for and applies any role-specific overrides. This approach is implemented in the get_applied_policies_for_asset_purpose method of AssetPolicyInference class.', 'purpose inference,policy engine,implementation'),
('Policy Inference Engines', 'Gap Analysis Algorithm', 'How does the policy gap analysis algorithm work?', 'The policy gap analysis algorithm in the Assets page compares sensitivity-based and purpose-based policy outcomes to identify gaps: (1) Parallel Inference - It runs both sensitivity-based and purpose-based inference engines for the same asset; (2) Policy Normalization - It normalizes the results into comparable formats, focusing on security controls like encryption and masking; (3) Comparison Logic - For each data element, it compares the controls derived from sensitivity against those derived from purpose using the _compare_policy_values method; (4) Gap Classification - It classifies each comparison as "Meets Requirements" (purpose-based controls match sensitivity requirements), "Exceeds Requirements" (purpose-based controls are stricter), or "Falls Short of Requirements" (sensitivity requires stronger controls than currently implemented); (5) Visualization - Results are displayed in a color-coded table highlighting gaps.', 'gap analysis,policy comparison,implementation'),
('Policy Inference Engines', 'Risk Assessment Algorithm', 'How does the risk assessment algorithm work?', 'The risk assessment algorithm in the Assets page evaluates potential risks if recommended security controls are not implemented: (1) Obligation Analysis - It starts with security and privacy obligations identified by the sensitivity-based inference engine; (2) Risk Mapping - For each obligation, it identifies potential risks using the obligation_risk mapping table; (3) Risk Classification - It categorizes risks by type (e.g., Data Breach, Regulatory, Reputational); (4) Impact and Likelihood Evaluation - It assesses each risk\'s impact and likelihood based on predefined values in the risk table; (5) Risk Rating Calculation - It combines impact and likelihood using a matrix: Critical (High+High), High (High+Medium, Medium+High), Medium (Medium+Medium, High+Low, Low+High), Low (other combinations); (6) Prioritization - Risks are sorted by severity for actionable recommendations.', 'risk assessment,algorithm,implementation'),
('Policy Inference Engines', 'Policy JSON Generation', 'How does the system generate structured policy JSON for implementation?', 'The system generates structured policy JSON through the analyze_policies_for_asset method in the Assets page: (1) Data Collection - It processes all policies applicable to an asset, organized by table, column, purpose, and policy type; (2) Hierarchical Organization - It structures the JSON with nested objects: tables → columns → purposes → policy types; (3) Policy Aggregation - For each column and purpose, it aggregates security, usage, and retention policies; (4) Role Integration - It includes role information for each purpose, identifying which roles can access the data; (5) Override Incorporation - It applies any role-specific overrides, marking them with an is_override flag; (6) Format Standardization - It ensures consistent formatting of policy attributes. This structured JSON can be used by the DDL Generator to create implementation scripts for target systems.', 'JSON generation,policy implementation,structured data');

-- Technical Implementation
INSERT INTO knowledge_base (category, subcategory, question, answer, tags) VALUES
('Technical Implementation', 'DDL Generation', 'How does the system generate DDL for implementing policies in Snowflake?', 'The DDL generation process in the application converts policy JSON into executable Snowflake Data Definition Language: (1) Policy Extraction - The DDLGenerator component takes the policy JSON output from the AssetPolicyInference engine; (2) Role Hierarchy Creation - It generates CREATE ROLE statements for purpose-based roles (PURPOSE_X) and data access roles (DATA_X_Y); (3) Masking Policy Creation - For columns requiring masking, it creates masking policies using Snowflake\'s MASKING POLICY syntax, with conditional logic based on IS_ROLE_IN_SESSION(); (4) Role Grants - It generates GRANT statements to establish the role hierarchy and column-level permissions; (5) Table Comments - It adds comments to tables and columns documenting the applied policies. The generator follows a consistent format that prioritizes maintainability, naming masking policies based on data element names for standardization.', 'DDL generation,Snowflake,implementation'),
('Technical Implementation', 'Policy Inference Class Structure', 'What is the class structure of the policy inference engine?', 'The policy inference engine is implemented through these key classes: (1) AssetPolicyInference - The main engine that determines how policies apply to assets, with methods for both sensitivity-based and purpose-based inference; (2) SensitivityInference - A specialized component that determines data element sensitivity levels; (3) GlossaryRepository - Provides access to core constructs like purposes, roles, and data elements; (4) RegulatoryMetadataRepository - Manages policy definitions and their relationships; (5) CatalogRepository - Provides information about data assets and their structure; (6) InventoryRepository - Manages the inventory of data assets. The AssetPolicyInference class contains the core inference algorithms, including get_applied_policies_for_asset_purpose and get_sensitivity_based_policies_for_asset.', 'class structure,architecture,implementation'),
('Technical Implementation', 'Performance Considerations', 'What performance considerations are implemented in the policy inference engine?', 'The policy inference engine in the application incorporates several performance considerations: (1) Query Optimization - It uses targeted database queries with proper indexing on key tables like policy_purpose_data_element; (2) Batch Processing - When processing multiple data elements, it uses batch operations rather than individual queries; (3) Query Reduction - It minimizes database round-trips by retrieving all necessary data in bulk operations. Additionally, the engine uses pandas DataFrames for efficient data manipulation and filtering during the inference process. These optimizations help the engine handle policy inference across data assets with many columns while maintaining responsive performance in the user interface.', 'performance,optimization,implementation'),
('Technical Implementation', 'Logging Implementation', 'How does the policy inference engine implement logging?', 'The policy inference engine implements logging through Python\'s standard logging module: (1) Operation Markers - Each major operation is marked with start/end log entries (e.g., "===== POLICY ANALYSIS STARTED ====="); (2) Parameter Logging - All input parameters are logged at the start of operations; (3) Decision Points - Key decision points in the inference algorithm are logged with their outcomes; (4) Result Summaries - Summary statistics of results are logged (e.g., "Generated JSON with X tables"); (5) Error Handling - Exceptions are caught and logged with detailed context. This logging implementation provides visibility into the policy inference process, helping with troubleshooting and providing an audit trail of policy decisions.', 'logging,debugging,implementation');
